...
public class UMAResourceRegistratorFilter implements ContainerRequestFilter {

    private final static Logger LOGGER = Logger.getLogger(UMAResourceRegistratorFilter.class.getName());
    ...
    public AuthToken convertToAuthToken(String accessToken, JsonObject userInfo) {
        ...
        //DONE lab11: Ex4: Read the field named "expires_in" from userInfo and store it in expiresIn variable. 
        //DONE lab11: Ex4: Use userInfo.get() method and cast it to JsonNumber -> this way there will not be an exception thrown if the value is missing.
        ...
        User user = User.builder()
                //DONE lab11: Ex4: Extract the user's full name from the field named "name". Use userInfo.getString(String fieldName) method.
                .uid(userInfo.getString("sub"))
                //DONE lab11: Ex4: Extract the user's full name from the field named "name"
                .name(userInfo.getString("name", null))
                //DONE lab11: Ex4: Extract the user's email from the field named "email"
                .email(userInfo.getString("email", null))
                //DONE lab11: Ex4: Extract the user's family name from the field named "family_name"
                .familyName(userInfo.getString("family_name", null))
                //DONE lab11: Ex4: Extract the user's given name from the field named "given_name"
